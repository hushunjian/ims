...
<body>
		...
		<section id="main-content">
										...
										<tbody id="tbody">
											<c:forEach items="${dataObj.result}" var="item">
												<tr class="gradeX">
													...
													<a href="javascript:changeTopicKingShow(${item.id },${admin_user.id },${item.uid })">更换国王</a>
			...
			<div class="modal-dialog modal-sm">
				...
				<div class="modal-header">
					...
					<h4 class="modal-title">更换国王</h4>
				</div>
				<div class="modal-body">
				<input type="hidden" id="topicId" name="topicId" value="0">
				<input type="hidden" id="sourceUid" name="sourceUid" value="0">
				<input type="hidden" id="optUid" name="optUid" value="0">
				<div class="form-group">
					<label for="NewTopicKingUid">新国王UID</label>
                       <input type="text" id="targetUid" name="targetUid" class="form-control" />
				</div>
				</div>
				<div class="modal-footer">
					<button class="btn btn-primary" data-dismiss="modal" aria-hidden="true" onclick="changeTopicKingNew();">保存</button>
	...
	<script type="text/javascript">
	function changeTopicKingShow(topicId,optUid,sourceUid){
		$('#modal').modal('show');
		$("#topicId").val(topicId);
		$("#optUid").val(optUid);
		$("#sourceUid").val(sourceUid);
	}
	
	function changeTopicKingNew(){
		var msg = "您确定要修改该王国国王吗？"; 
		if(confirm(msg)){
			var param = {sourceUid:$("#sourceUid").val(),optUid:$("#optUid").val(),targetUid:$("#targetUid").val(),topicId:$("#topicId").val()};
			 $.post("./changeTopicKingNew",param,function(data){
				 if(data=='1'){
            		  alert('操作成功');
            		  location.reload();
            		  $('#modal').modal('hide');
