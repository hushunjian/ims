...
<body>
		...
		<section id="main-content">
			...
			<div class="modal-dialog modal-sm">
				...
				<input type="hidden" id="id" name="id" value="0">
					...
					<div class="form-group">
						<label for="exampleInput">类型</label>
                        <select class="form-control" name="type" id="type">
                        	<option value="0">首页和标签</option>
	...
	var sourceTable=$('#mytable').DataTable( {
	    ...
	    "columns": [
	        ...
	        {title:"操作",orderable:false,width:200,render:function(data, type, row, meta){
	        	...
	        	txt+='<a class="btn btn-primary btn-xs " href="./listAdTag?bannerId='+row.id+'">关联标签列表</a>'
	        	return txt;
	        }}
	     ],
	...
	function del(id){
		 ...
		 if (confirm(msg)){ 
			 var param = {id:id};
			 $.post("./delAdBanner",param, function(data){
				 if(data=='1'){
	...
	function addAdBannerShow(){
		...
		$("#adBannerName").val('');
		$("#bannerPosition").val('');
		$("#adBannerHeight").val(0);
		$("#adBannerWidth").val(0);
		$("#type").val('');
	}
	   function addAdBanner(){
				 var param = {id:$("#id").val(),adBannerName:$("#adBannerName").val(),bannerPosition:$("#bannerPosition").val(),type:$("#type").val(),adBannerHeight:$("#adBannerHeight").val(),adBannerWidth:$("#adBannerWidth").val()};
				 $.post("./addAdBanner",param,function(data){
					 if(data=='1'){
	            		  //sourceTable.ajax.reload();
	            		  alert('操作成功');
	            		  sourceTable.draw(false);
	   ...
	   function editAdBanner(id){
				 var param = {id:id};
				 $.post("./getAdBanner",param,function(data){
					 if(data!=null & data!=''){
	            		  $("#id").val(data.id);
