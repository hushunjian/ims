...
public class UserLogAnalyzeTask{
	...
	public void countUserTagLikeDay() {
		...
		for(Map.Entry<Long, TagScoreAnalyzer> entry: userCounterMap.entrySet()){
			...
			//先把该用户的所有行为衰减10%
			this.decayUserTagScore(uid);
			//删除该用户已经衰减到0的标签
			this.delete0Score(uid);
			//将计算的增量更新到库中
			...
			//将分值大于20分的标签+兴趣更新到标签中间表user_tag
			//先删除原有中间表内容
			this.deleteUserTagByUid(uid);
			//再将新的结果集更新到user_tag
			this.insertUserTag(uid);
		}
	}
    /**
     * 查用户访问日志
     ...
     * @return
     */
	private List<Map<String,Object>> getUserVisitLogByDay(String day,int skip,int limit){
    ...
    private boolean existsUserTagLike(long uid,String tag){
    ...
    private void decayUserTagScore(long uid){
    	String sql = "update user_tag_like set score=score*0.9,last_update_time=now() where uid=?";
    	jdbcTemplate.update(sql, uid);
    }
    
    private void delete0Score(long uid){
    	String sql = "delete from user_tag_like where uid=? and score<=0";
    	jdbcTemplate.update(sql, uid);
    }
    
    private void deleteUserTagByUid(long uid){
    	String sql = "delete from user_tag where uid=? and type=0";
    	jdbcTemplate.update(sql, uid);
    }
    
    private void updateUserTagLike(long uid, String tag, int score) {
	...
	private void insertUserTag(long uid){
		StringBuilder sb = new StringBuilder();
