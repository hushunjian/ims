...
public class KingdomPriceTask {
	...
	public void init(){
		...
		weightKeyList.add("ALGORITHM_HOT_V_PRICE_INCR_LIMIT");
	}
	...
	public void executeIncr(String dateStr) throws Exception{
		...
		int priceIncrLimit = this.getIntegerConfig("ALGORITHM_PRICE_INCR_LIMIT", weightConfigMap, 500);//普通王国价值增长上限
		int hotPriceIncrLimit = this.getIntegerConfig("ALGORITHM_HOT_V_PRICE_INCR_LIMIT", weightConfigMap, 800);//热点及大V王国价值增长上限
...
//			}
			...
			for(Map.Entry<String, KingdomCount> entry : kingCountMap.entrySet()){
				...
				if(null == topicData || isCreateTime){//当天新增的王国
					...
					if(kc.getIsHot() != 1 && !kc.isVlv()){//不是热点并且不是大V
						if((int)kv > priceIncrLimit){
							kc.setOverLimitPrice((int)kv-priceIncrLimit);
							kv = priceIncrLimit;
						}
					}else{//热点或大V
						if((int)kv > hotPriceIncrLimit){
							kc.setOverLimitPrice(hotPriceIncrLimit-(int)kv);
							kv = hotPriceIncrLimit;
						}
					}
					...
					if(kc.getIsHot() != 1 && !kc.isVlv()){//增长上限，当然热点王国和大V是不会限制的
						...
						if(_kv > hotPriceIncrLimit){
							kc.setOverLimitPrice(hotPriceIncrLimit-_kv);
							_kv = hotPriceIncrLimit;
						}
					}
