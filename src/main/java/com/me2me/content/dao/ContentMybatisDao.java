...
//            example.or(criteria2);
	...
	public int delAdBanner(long adBannerid){
		return adBannerMapper.deleteByPrimaryKey(adBannerid);
	}
	/**
	 * 获取广告位
	 * @param long id
	 ...
	 * @return
	 */
	public int getAdInfoCount(long adBannerId){
	 ...
	 * @return
	 */
	public List<AdInfo> getAdInfoList(long bannerId,int start,int pageSize){
		AdInfoExample example = new AdInfoExample();
		AdInfoExample.Criteria criteria = example.createCriteria();
		if(bannerId!=0){
	...
	public int delAdInfo(long adInfoId){
		return adInfoMapper.deleteByPrimaryKey(adInfoId);
	}
	/**
	 * 获取广告信息
	 * @param id
